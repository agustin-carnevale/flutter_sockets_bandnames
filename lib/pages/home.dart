...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    socketService.listen('active-bands', _handleActiveBands);
    super.initState();
  }

  void _handleActiveBands(dynamic bands) {
  ...
  Widget _bandTile(Band band) {
    ...
    return Dismissible(
      ...
      onDismissed: (_) =>
          socketService.emit('delete-band', {'bandId': band.id}),
      ...
      child: ListTile(
        ...
        onTap: () => socketService.emit('vote-band', {'bandId': band.id}),
      ),
    );
  }
  ...
  void addBandToList(String name) {
    if (name.length > 1) {
      final socketService = Provider.of<SocketService>(context, listen: false);
      socketService.emit('add-band', {'bandName': name});
    }
